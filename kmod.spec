...
Source0  : https://www.kernel.org/pub/linux/utils/kernel/kmod/kmod-34.2.tar.gz
Summary  : Tools to deal with kernel modules
...
%prep
%setup -q -n kmod-34.2
cd %{_builddir}/kmod-34.2
pushd ..
cp -a kmod-34.2 build32
popd
pushd ..
cp -a kmod-34.2 buildavx2
popd
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1743171561
unset LD_AS_NEEDED
...
export GOAMD64=v2
%reconfigure --disable-static --enable-tools \
...
--disable-test-modules \
--with-zstd \
--disable-manpages  --libdir=/usr/lib32 --build=i686-generic-linux-gnu --host=i686-generic-linux-gnu --target=i686-clr-linux-gnu
make  %{?_smp_mflags}  LIBS=-lpthread
popd
...
ln -sf kmod %{buildroot}/usr/bin/depmod
ln -sf kmod %{buildroot}/usr/bin/insmod
ln -sf kmod %{buildroot}/usr/bin/lsmod
ln -sf kmod %{buildroot}/usr/bin/modinfo
ln -sf kmod %{buildroot}/usr/bin/modprobe
ln -sf kmod %{buildroot}/usr/bin/rmmod
## install_append end
...
/usr/lib32/pkgconfig/32kmod.pc
/usr/lib32/pkgconfig/32libkmod.pc
/usr/lib32/pkgconfig/kmod.pc
/usr/lib32/pkgconfig/libkmod.pc

%files lib
%defattr(-,root,root,-)
/V3/usr/lib64/libkmod.so.2.5.1
/usr/lib64/libkmod.so.2
/usr/lib64/libkmod.so.2.5.1

%files lib32
%defattr(-,root,root,-)
/usr/lib32/libkmod.so.2
/usr/lib32/libkmod.so.2.5.1
